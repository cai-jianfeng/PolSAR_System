...
ResNet_model = ResNet.Net()
device = torch.device('cude:0' if torch.cuda.is_available() else 'cpu')
ResNet_model.to(device=device)

# criterion = torch.nn.MultiLabelSoftMarginLoss()
criterion = torch.nn.CrossEntropyLoss()
optimizer = optim.SGD(params=ResNet_model.parameters(), lr=train_parameters['learning_strategy']['lr'], momentum=0.5)
...
if __name__ == '__main__':
    ...
    for epoch in range(train_parameters['num_epochs']):
        CNN_train.train(model=ResNet_model,
                        epoch=epoch,
                        ...
                        criterion=criterion,
                        cost=cost)
        CNN_test.test(model=ResNet_model,
                      test_loader=test_loader,
                      device=device,
                      accuracy=accuracy)
    
    torch.save(ResNet_model.state_dict(), './ResNet_model_parameter.pkl')

    plot_mode = plot_mode()
    plot_mode.plt_image(title='ResNet',
                        x_data=list(range(len(cost))),
                        y_data=cost,
                        label='ResNet',
                        xlabel='num_data / batch_size * epoch',
                        ylabel='loss for whole dataset',
                        save_path='../plot/loss/lossFlevoland4_ResNet.png',
                        color='r')
    plot_mode.plt_image(title='ResNet',
                        ...
                        save_path='../plot/accuracy/accuracy_Flevoland4_ResNet.png',
                        color='r')
