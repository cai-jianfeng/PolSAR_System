...
from data_process.dataset2 import PolSARDataset
from DCNN import DCNN_without_label0
...
train_parameters = {
    ...
    "target_path": '../data_patch/T_RI_7',  # 数据集的路径
    "num_epochs": 10,  # 训练轮数
...
train_list_path = '../data_patch/T_RI_7/train.txt'
eval_list_path = '../data_patch/T_RI_7/eval.txt'
patch_size = train_parameters['input_size'][1:3]
predict_path = '../data_patch/T_RI_7/predict.txt'
'''
划分训练集和验证集, 乱序, 生成数据列表
'''
if os.path.getsize(target_path) == 0:
    ...
    # 分割数据集, 生成数据列表
    data = Data()
    data.save_data_label_segmentation_TRI_without_label0(data_path=data_path,
...
train_dataset = PolSARDataset(data_path='../data_patch/T_RI_7',
                              mode='train',
                              transform=transform)
test_dataset = PolSARDataset(data_path='../data_patch/T_RI_7',
                             mode='eval',
                             transform=transform)
...
DCNN_model = DCNN_without_label0.Double_CNN()
...
if __name__ == '__main__':
    ...
    for epoch in range(train_parameters['num_epochs']):
        ...
        CNN_test.test(model=DCNN_model,
                      ...
                      accuracy=accuracy)
    # 保存模型参数
    torch.save(DCNN_model.state_dict(), "./DCNN_model_parameter_whole_dataset_10%_10_7x7.pkl")
    
    plt.plot(list(range(len(cost))), cost, 'r', label='DCNN')
    plt.ylabel('loss for whole dataset')
    plt.xlabel('num_data / batch_size * epoch')
    plt.grid()
    plt.savefig('../plot/loss/loss_Flevoland4_DCNN_whole_dataset_10%_10_7x7.png')
    plt.show()
    ...
    plt.xlabel('epoch')
    plt.grid()
    plt.savefig('../plot/accuracy/accuracy_Flevoland4_DCNN_whole_dataset_10%_10_7x7.png')
    plt.show()
