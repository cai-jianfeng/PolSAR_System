...
from SNN.ResNet import Net
from data_process.data_preprocess import Data
import os

ResNet_model = Net()
ResNet_parameters = torch.load('../SNN/ResNet_model_parameter_whole_dataset_10%_10.pkl')  # 加载训练好的模型参数
ResNet_model.load_state_dict(ResNet_parameters)
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
ResNet_model.to(device=device)
data_path = '../data/prepro_flevoland4/pre_data/TR.xlsx'
data_paths = []
data = Data()
target_path = '../data_patch/T_R'
...
label_pic_name = 'flevoland4_ResNet_10%_10'
transform = transforms.Compose([
    transforms.ToTensor(),
])
predict_model = CNN_predict()
predict_model.predict(model=ResNet_model,
