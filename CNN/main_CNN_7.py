...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(train_parameters['num_epochs'])):
        ...
        CNN_test.test(model=CNN_model,
                      ...
                      accuracy=accuracy)
    # 保存模型参数
    torch.save(CNN_model.state_dict(), "./CNN_model_parameter_whole_dataset_10%_10_7x7.pkl")

    plt.plot(list(range(len(cost))), cost, 'r', label='CNN')
    plt.ylabel('loss for whole dataset')
    plt.xlabel('num_data / batch_size * epoch')
    plt.grid()
    plt.savefig('../plot/loss/loss_whole_Flevoland4_CNN_10%_10_7x7.png')
    plt.show()
    ...
    plt.xlabel('epoch')
    plt.grid()
    plt.savefig('../plot/accuracy/accuracy_whole_Flevoland4_CNN.png_10%_10_7x7')
    plt.show()
